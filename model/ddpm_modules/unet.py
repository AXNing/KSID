...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb):
        h = self.block1(x)#分组归一化、Swish激活函数、可选的Dropout层以及卷积层
        h = self.noise_func(h, time_emb)#时间信息,噪声嵌入对输入特征图进行特征维度上的仿射或加性变换。它可以在模型中用于对输入进行动态调整，使得模型能够更好地适应不同的输入噪声或环境变化
        h = self.block2(h)
