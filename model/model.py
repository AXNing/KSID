...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        self.netG = self.set_device(networks.define_G(opt))#定义了一个G网络，若是uncertainty_train训练则是diffusion_Pt，否则diffusion
        if opt['dist']:
            self.netG.to(device)
        ...
        if not opt['uncertainty_train'] and self.opt['phase'] == 'train':
            self.netGU.load_state_dict(torch.load(self.opt['path']['resume_state']+'_gen.pth'), strict=True) # use uncertainty model for initialization #如果不是uncertainty_train训练，则导入uncertainty模型
            if opt['dist']:
    ...
    def optimize_parameters(self):
        ...
        if not self.opt['uncertainty_train']:
            if self.opt['dist']:
                ...
                l_pix, l_gsad,l_fre = self.netG(self.data, self.netGU.denoise_fn)
            ...
            l_fre = l_fre/int(b*c*h*w)
            # l_svd = l_svd.sum()/int(b*c*h*w)
            loss = l_pix + l_gsad +l_fre
            loss.backward()
            self.optG.step()
            ...
            self.log_dict['l_fre'] = l_fre.item()
        else:
